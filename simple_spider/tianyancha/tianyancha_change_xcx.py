...
headers = {
    ...
    , 'Referer': 'https://servicewechat.com/wx9f2867fc22873452/18/page-frame.html'
    , 'version': 'TYC-XCX-WX'
...
def get_detail(url, fail_cnt=0):
    ...
    while 1:
        try:
            ...
            change_ip()
    try:
        if resp.text and '"state":"warn"' in resp.text:
        ...
        if not resp.text or '"state":"ok"' not in resp.text:
            ...
            #     get_detail(re.sub('pageNum=(\d+)', lambda g: 'pageNum=' + str(int(g.group(1)) + 1), url))
    except:
        traceback.print_exc()
...
def save_to_db(item):
    while 1:
        try:
            db['tianyancha_level2_' + time.strftime("%Y%m%d%H", time.localtime())].save(item)
            break
        except Exception, e:
            traceback.print_exc()
            log("db error " + str(e))
...
log_file = open('tianyancha_change_xcx.log', 'a')
...
if __name__ == '__main__':
    init_db()
    task_key = 'task_1711_lv2'
    page_size = 50
    url_prefix = 'https://api9.tianyancha.com/services/v3/expanse/changeinfoEm3?id=%s&pageNum=1&pageSize=' \
                 + str(page_size)
    try:
        while True:
            ...
            if urls:
                ...
                for link in urls:
                    while 1:
                        try:
                            if link:
                                get_detail(url_prefix % link.strip())
                            break
                        except:
                            init_db()
