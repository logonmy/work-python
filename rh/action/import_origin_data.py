...
class Controller:
    ...
    def get_insert_sql(self, table, csv_columns, column_map):
        insert_sql = "insert into " + table
        self.log('csv_column: ' + str(csv_columns))
        insert_columns_str = ''
        ...
        self.log('insert_sql: ' + insert_sql)
        return insert_sql, _columns

    def insert_records(self, records, columns, db, cursor, insert_sql):
        ...
        for i, fields in enumerate(records):
            ...
            if (i + 1) % batch_insert_cnt == 0:
                try:
                    ...
                    self.log(
                        'insert error in rows from %d to %d, error: %s' % ((i + 1 - batch_insert_cnt), (i + 1), msg))
                items = []
                if (i + 1) % batch_commit_cnt == 0:
                    db.commit()
                    now_time = time.clock()
                    self.log('insert %d rows elapse %d s' % (batch_commit_cnt, now_time - begin_time))
                    begin_time = now_time
        else:
            if items:
                cursor.executemany(insert_sql, items)
    ...
    def remove_duplicate(self, db, cursor):
        self.log('执行排重...')
        cursor.execute("""
    ...
    def main(self, origin_file):
        ...
        try:
            with open(origin_file, 'rb') as csvfile:
                ...
                insert_sql, _column = self.get_insert_sql('rh_err_origin', csv_reader.fieldnames, column_map)
                cursor = db.get_cursor()
                # 插入数据库
                self.insert_records(csv_reader, _column, db, cursor, insert_sql)
            # 去重
            self.remove_duplicate(db, cursor)
        except Exception, e:
            msg = e.message if e.message else str(e.args)
            self.log("error>>>" + msg)
            # raise Exception(msg)
