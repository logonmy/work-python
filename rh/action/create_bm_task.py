...
class Controller:
    ...
    def insert_records(self, records, db, cursor):
        insert_sql = """
    ...
    def update_origin(self, gids, db, cursor):
        self.log("update origin table...")
        update_sql = 'update rh_err_origin set is_task = 1 where gid = %s'
        cursor.executemany(update_sql, gids)
        db.commit()

    def main(self, begin_day, end_day):
        global config
        ...
        try:
            ...
            if rows:
                ...
                for r in rows:
                    ...
                    items.append(
                        (r['group_group'], r['STANDARD'], r['zc_code'], '1', r['city_code'], r['rh_tc'], '', x, y,
                         r['keyword'], '', task_id, task_id, r['group_group'], '标准地址'))
                self.insert_records(items, db, cursor)
                self.update_origin(gids, db, cursor)
            else:
                self.log("没有查询到任务数据!!!")
